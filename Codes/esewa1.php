...
if (isset($_GET['pid'])) {
    $pid = rtrim($_GET['pid'],';');
        ...
        $sql="SELECT *from cart inner join uploads on cart.id=uploads.id Where cart.id='$pid'";
        $r=$conn->query($sql);
        if($row=$r->fetch_assoc()){
    $pname=$row['pname'];
    $pprice=$row['pprice']*$row['qty'];
    $serchar=$row['serchar']*$row['qty']; 
    $tax=$row['tax']*$row['qty'];
    
    
    $total = $pprice + $tax + $serchar + 0; 
    
    $pprice  = number_format($pprice, 2, '.', '');
    $serchar = number_format($serchar, 2, '.', '');
...
<?php
 ...
 <input type="text" id="success_url" name="success_url" value="https://localhost/projectII/Codes/sucess.php" required>
 <input type="text" id="failure_url" name="failure_url" value="https://localhost/projectII/Codes/supermain.php" required>
